...
class NnovoPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `novo`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM novo WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO novo (date, title, link, content)"
